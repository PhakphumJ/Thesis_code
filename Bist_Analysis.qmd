...
ii) 2565: (D01 or D03)  / Number of employees

9. K/L
i) 2555: f06 / number of employees
ii) 2565: E08 / number of employees
...
### Calculate the sales per employee
...
BICTS_2555_sales_filtered <- BICTS_2555 %>% 
  filter(sales > 12000)

# Calculate the sales per employee
BICTS_2555_sales_filtered <- BICTS_2555_sales_filtered %>% 
  mutate(sales_per_employee = sales / num_employ)
...
## Drop those with sales < 12,000
BICTS_2565_sales_filtered <- BICTS_2565 %>% 
  filter(sales > 12000)

# Calculate the sales per employee
BICTS_2565_sales_filtered <- BICTS_2565_sales_filtered %>% 
  mutate(sales_per_employee = sales / num_employ)
...
## There seems to be outlier in the fixed assets. So, I will drop those with top 0.5% of fixed assets.

# 2555
# Replace missing values with 0
BICTS_2555_KL_filtered <- BICTS_2555 %>% 
  mutate(f06 = ifelse(is.na(f06), 0, f06))

# Drop those with fixed assets < 6,000
BICTS_2555_KL_filtered <- BICTS_2555_KL_filtered %>% 
  filter(f06 > 6000)

# Drop those with fixed assets that are in the top 0.5
BICTS_2555_KL_filtered <- BICTS_2555_KL_filtered %>% 
  filter(f06 < quantile(f06, 0.995))
...
# 2565
# Replace missing values with 0
BICTS_2565_KL_filtered <- BICTS_2565 %>% 
  mutate(E08 = ifelse(is.na(E08), 0, E08))

# Drop those with fixed assets < 6,000
BICTS_2565_KL_filtered <- BICTS_2565_KL_filtered %>% 
  filter(E08 > 6000)

# Drop those with fixed assets that are in the top 0.5
BICTS_2565_KL_filtered <- BICTS_2565_KL_filtered %>% 
  filter(E08 < quantile(E08, 0.995))
...
```{r}
## Categorize the number of employees
# 2555
BICTS_2555 <- BICTS_2555 %>% 
...
# 2555
Avg_KL_2555 <- BICTS_2555_KL_filtered %>% group_by(reg) %>% summarise(avg_KL = weighted.mean(KL, wgt5 , na.rm = TRUE))

# 2565
Avg_KL_2565 <- BICTS_2565_KL_filtered %>% group_by(REG) %>% summarise(avg_KL = weighted.mean(KL, WGT , na.rm = TRUE))
```

### Sales per employee

```{r}
### Calculate the average sales per employee by region
# 2555
Avg_sales_per_employee_2555 <- BICTS_2555_sales_filtered %>% group_by(reg) %>% summarise(avg_sales_per_employee = weighted.mean(sales_per_employee, wgt5 , na.rm = TRUE))

# 2565
Avg_sales_per_employee_2565 <- BICTS_2565_sales_filtered %>% group_by(REG) %>% summarise(avg_sales_per_employee = weighted.mean(sales_per_employee, WGT , na.rm = TRUE))
...
Emp_Num_LF_scaled %>% 
  gt(rowname_col = "REG", groupname_col = "Year") %>% cols_label(avg_num_employ = "Average number of employees", num_firms = "Number of firms (in 10,000)", Labor_force = "Labor force (in million)") %>% cols_align(align = "center") %>% cols_width(everything() ~ px(160)) %>% fmt_number(decimals = 2)


```

### Sales per employee and K/L
...
```{r}
### Combine sales per employee of all years
# First, add year to the tables
Avg_sales_per_employee_2555 <- Avg_sales_per_employee_2555 %>% 
  mutate(Year = 2011)
...
Avg_sales_per_employee_2565 <- Avg_sales_per_employee_2565 %>%
  mutate(Year = 2021)

# Combine the two tables
Avg_sales_per_employee <- rbind(Avg_sales_per_employee_2555, Avg_sales_per_employee_2565)
```

```{r}
## Combine the two tables
Sales_KL <- Avg_sales_per_employee %>% 
...
## Let's try creating the table with gt.
Sales_KL %>% 
  gt(rowname_col = "REG", groupname_col = "Year") %>% cols_label(avg_sales_per_employee = "Average sales per employee", avg_KL = "Average K/L") %>% cols_align(align = "center") %>% cols_width(everything() ~ px(160)) %>% fmt_number(decimals = 2)
...
# Plot
BICTS_2555 %>% 
  ggplot(aes(x = SIZE_R, weight = wgt5, y = after_stat(prop))) + geom_bar(stat = "prop") + facet_rep_wrap(~REG, labeller = region_labeller, ncol = 1) + labs(title = "Distribution of Firm Size (by region)", x = "Number of Employees", y = "Proportion")
```
```{r}
## 2565
# Plot
BICTS_2565 %>% 
  ggplot(aes(x = SIZE_R, weight = WGT, y = after_stat(prop))) + geom_bar(stat = "prop") + facet_rep_wrap(~REG, labeller = region_labeller, ncol = 1) + labs(title = "Distribution of Firm Size (by region)", x = "Number of Employees", y = "Proportion")
...
### Mean K/L and Sales per employee (by region)
...
```
```{r}
### Plot Output per employee
Sales_per_employee_plot <- ggplot(Sales_KL, aes(x = Year, y = avg_sales_per_employee, color = REG)) + geom_line() + labs(x = "Year", y = "Average Sales per Employee (THB)") + theme(legend.position = "bottom") + theme(legend.position = "bottom") + scale_color_manual(values = c("red", "blue", "green", "purple", "orange")) + theme(legend.position = "bottom", legend.title = element_blank()) + scale_y_continuous(labels = scales::comma) + scale_x_continuous(breaks = c(2011, 2021))

Sales_per_employee_plot
```

```{r}
## Combine the two plots
K_LL_plot + Sales_per_employee_plot + plot_annotation(title = "Mean K/L and Mean Sales per employee (by region)") + plot_layout(guides = "collect") & theme(legend.position = 'bottom')
