...
Before analysing, I will trim the data by removing firms with sales lower than 12,000, fixed assets lower than 6,000, firms with zero workers.
...
```{r}
## 2550
INDUS_2550 %>% distinct(num_employ, .keep_all = TRUE)
...
### K/L and Value-added per worker
...
```{r}
### Combine value-added per worker of two years
# First, add year to the tables
...
```{r}
## Let's try creating the table with gt.
KLVAPerEmp %>% gt(rowname_col = "REG", groupname_col = "Year") %>% cols_label(Avg_Capital_to_Labor = "Average K/L", Avg_Value_Added_per_emp = "Average Value-added per worker") %>% cols_align(align = "center") %>% cols_width(everything() ~ px(160)) %>% fmt_number(decimals = 0)
...
# Plot
ggplot(INDUS_2560_VA_filtered, aes(x = ln_Value_Added_per_emp, weight = WGT,  group = REG, color = REG)) + geom_density(warnWbw = FALSE) + labs(title = "Distribution of value-added per worker (by region) in 2016", x = "ln(Value-added per worker)", y = "Density") + theme(legend.position = "bottom") + scale_color_manual(values = c("red", "blue", "green", "purple", "orange"), labels = c( "Bangkok", "Central", "North", "Northeast", "South")) + theme(legend.position = "bottom", legend.title = element_blank())
...
### Mean K/L and Mean Value-added per worker (by region)
...
## Firm size vs K/L
# 2565
ggplot(INDUS_2565, aes(x = Capital_to_Labor, y = num_employ , weight = WGT)) + geom_point() + ylim(0,5000) + xlim(0,50000000)

```

```{r}
## Firm size vs Value-added per worker
# 2565
ggplot(INDUS_2565_VA_filtered, aes(x = Value_Added_per_emp, y = num_employ , weight = WGT)) + geom_point() + ylim(0,5000) + xlim(0,50000000)
...
```{r}
### Let's examine the role of A03 (Form of economic organization).
# 2565
# Make A03 as character
INDUS_2565$A03 <- as.character(INDUS_2565$A03)

ggplot(INDUS_2565, aes(x = Capital_to_Labor, y = num_employ , weight = WGT, color = A03)) + geom_point() + ylim(0,2000) 
...
```{r}
### Let's examine the role of A01 (Form of legal organization).
# 2565
# Make A01 as character
INDUS_2565$A01 <- as.character(INDUS_2565$A01)

# Don't plot A01 = 4 (Government, State-enterprises)
ggplot(INDUS_2565 %>% filter(A01 != "4"), aes(x = Capital_to_Labor, y = num_employ , weight = WGT, color = A01)) + geom_point() + ylim(0,2000) 
