...
INDUS_2550 %>% filter(num_employ > 2500)
```
...
# 2565
# First, have to bin sales (D01). Want 50 bins.
INDUS_2565 <- INDUS_2565 %>% mutate(Sales_bin = ntile(D01, 50))
...
# Plot
Sales_and_num_work_plot_raw <- ggplot(Num_emp_and_sales_bin, aes(x = Sales_bin, y = Avg_num_emp)) + geom_point() + labs(title = "Mean of number of workers in each sales bin", x = "Sales bin", y = "Average number of workers")

Sales_and_num_work_plot_raw
```

```{r}
### Plot weighted mean of number of workers in each sales bin. But limit y-axis to 200.

Sales_and_num_work_plot_cut <- ggplot(Num_emp_and_sales_bin, aes(x = Sales_bin, y = Avg_num_emp)) + geom_point() + labs(title = "Mean of number of workers in each sales bin", x = "Sales bin", y = "Average number of workers") + ylim(0, 200)

Sales_and_num_work_plot_cut
```
...
# 2565
# First, have to bin K/L. Want 50 bins.

INDUS_2565 <- INDUS_2565 %>% mutate(KL_bin = ntile(Capital_to_Labor, 100))
...
# Plot
KL_and_num_work_plot_raw <- ggplot(Num_emp_and_KL_bin, aes(x = KL_bin, y = Avg_num_emp)) + geom_point() + labs(title = "Mean of number of workers in each K/L bin", x = "K/L bin", y = "Average number of workers")
...
# 2565
# First, have to bin value-added per worker. Want 50 bins.
INDUS_2565 <- INDUS_2565 %>% mutate(VA_bin = ntile(Value_Added_per_emp, 100))
...
# Plot
VA_and_num_work_plot_raw <- ggplot(Num_emp_and_VA_bin, aes(x = VA_bin, y = Avg_num_emp)) + geom_point() + labs(title = "Mean of number of workers in each value-added per worker bin", x = "Value-added per worker bin", y = "Average number of workers")
