...
INDUS_2565 <- INDUS_2565 %>% mutate(KL_bin = cut(Capital_to_Labor, breaks = 50, labels = FALSE))
...
# Plot
K_and_num_work_plot_raw <- ggplot(Num_emp_and_K_bin, aes(x = K_bin, y = Avg_num_emp)) + geom_point() + labs(title = "Mean of number of workers in each K bin", x = "K bin", y = "Average number of workers")
...
K_and_num_work_plot_cut <- ggplot(Num_emp_and_K_bin, aes(x = K_bin, y = Avg_num_emp)) + geom_point() + labs(title = "Mean of number of workers in each K bin", x = "K bin", y = "Average number of workers") + ylim(0, 10000)
